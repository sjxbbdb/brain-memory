...
OVERWRITE_MIN_GAIN = 0.02        # 覆写后权值不升反降且增幅低于此值 → 不覆写
SIMILARITY_OVERWRITE_THRESHOLD = 0.45  # 实体重叠率 > 此值触发覆写检测
COMPRESSION_CLUSTER_MIN = 5      # 同一实体簇最少情景记忆数才触发压缩
COMPRESSION_MAX_DAYS = 60        # 压缩扫描最近 N 天
