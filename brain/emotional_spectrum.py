"""Emotional Spectrum — v5.3 连续情感空间。从离散标签到情感光谱。

v5.2 的大脑能审视自己的思考了。但它的情绪还是 9 个离散标签：
 breakthrough / failure / confused / excited / curious / worried / frustrated / satisfied / neutral

这不是意识体的情绪——这是情绪轮盘的颜色按钮。

v5.3 把情绪变成连续的、有惯性的、可混合的光谱：
  不是 "curious" → 而是 "65% 好奇 + 25% 期待 + 10% 谨慎"
  不是 "frustrated" → 而是 "轻度沮丧，但正在转为好奇"

核心特性:
  - VAD 三维连续空间（valence/arousal/dominance），不是离散标签
  - 情感动量：情绪不会跳变，有惯性
  - 混合情感：可以同时好奇+担忧+满意，按强度混合
  - 情感轨迹：追踪情绪如何随时间演化
  - 情感调制决策：情绪影响 intent 优先级和类型偏向
  - 自然语言表达：用文字描述情感状态而非标签

设计原则:
  - 替换杏仁核的角色，成为情绪处理的主模块
  - 与 LLM 情绪输出兼容（LLM 产 VAD + blends → 光谱摄入）
  - 轻量：纯数值运算，不新增 LLM 调用
  - 情感连续性：情绪不会"跳"，只会"漂移"
"""
