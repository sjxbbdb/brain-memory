...
class BrainStem:
    ...
    async def _tick(self):
        ...
        try:
            ...
            # Replace working memory with session's (keep global WM for idle ticks)
        ...
        if thalamus_out["has_input"] and not thalamus_out["discarded"]:
            ...
            gate = gate_check(
                ...
                novelty=0.5,
                goal_relevance=0.6 if self.state.current_goal else 0.4,
            ...
            if is_external and gate["passed"]:
                try:
                    ...
                    if llm_emotion:
                        ...
                        self.amygdala.valence = prev_v * 0.3 + v * 0.7
                        self.amygdala.arousal = prev_a * 0.3 + a * 0.7
                        self.amygdala.dominance = prev_d * 0.3 + d * 0.7
                    ...
                    logger.warning("brain-stem: unified LLM call failed: %s", str(e)[:80])
        ...
        if input_data:
            ...
            # Save back working memory to session
            session.working_memory = self.working_memory
    ...
    async def _reflection_tick(self):
        ...
        if self.state.total_ticks % 150 == 0:
