"""Metacognition — v5.2 元认知。大脑对自己思考的思考。

v5.1 大脑能自己设定目标并行动了。但它在行动时不知道自己：
  - 是否过度自信（90% confidence 但每次都错）
  - 是否陷入循环（反复做同一件事没进展）
  - 是否认知过载（处理太多信息，思考质量下降）
  - 是否有盲区（有些话题一直在回避）

元认知模块是大脑的"内部审计员"——不参与决策，只评价决策质量。
它的输出影响自我模型和未来决策，但它不直接做决策。
...
logger = logging.getLogger("brain-v5.metacognition")
...
class Metacognition:
    ...
    def update_cognitive_load(self, llm_called: bool, input_processed: bool, recent_input_count: int):
        """每个 tick 更新认知负荷。"""
        # 强度：综合 LLM 调用 + 输入处理
        intensity = 0.0
        if llm_called:
