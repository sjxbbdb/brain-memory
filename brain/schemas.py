...
class MemoryResponse(BaseModel):
    ...
    # v3.0 覆写权值字段
    overwrite_weight: float = 0.5
    compressed_to: list[str] = []
...
def row_to_response(row: dict) -> MemoryResponse:
    ...
    return MemoryResponse(
        ...
        consolidated_to=safe_json(row.get("consolidated_to"), []),
        last_consolidated=row.get("last_consolidated"),
        tags=safe_json(row.get("tags"), []),
        entities=safe_json(row.get("entities"), []),
        relations=safe_json(row.get("relations"), []),
        contradicted_by=safe_json(row.get("contradicted_by"), []),
        supersedes=safe_json(row.get("supersedes"), []),
        review_needed=bool(row.get("review_needed", 0)),
        archived=bool(row.get("archived", 0)),
        current_strength=None,
        # v3.0
        overwrite_weight=row.get("overwrite_weight", 0.5),
        compressed_to=safe_json(row.get("compressed_to"), []),
        parent_id=row.get("parent_id"),
        region_trace=safe_json(row.get("region_trace"), []),
