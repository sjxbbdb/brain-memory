"""Amygdala — V6 杏仁核。情绪标记 + 突显性检测（规则引擎 → State Transformer）。

V6 升级: 不再维护自己的实例变量作为状态源。
  读取 ActivationField 的当前 VAD 值，
  输出 delta 写入 ActivationField，
  实例变量保留为向后兼容缓存。

职责:
  1. 检测输入文本的情绪信号
  2. 计算目标 VAD 向量
  3. 写入 ActivationField（带惯性平滑）
  4. 计算突显度 (salience)
"""
...
class Amygdala:
    ...
    V6: 可选的 activation 参数将结果写入 ActivationField。
    向后兼容: 无 activation 时行为与 v5.4 一致。
    """
    ...
    def evaluate(self, text: str, current_state: dict, activation=None) -> dict:
        ...
        # V6: 优先从 ActivationField 读取当前值
        if activation is not None:
            prev_v = activation.get("valence")
            prev_a = activation.get("arousal")
            prev_d = activation.get("dominance")
        else:
            prev = current_state.get("emotion_vector", {})
            prev_v = prev.get("valence", 0.5)
            prev_a = prev.get("arousal", 0.5)
            prev_d = prev.get("dominance", 0.5)

        new_v = prev_v * EMOTION_DECAY_RATE + target_valence * (1 - EMOTION_DECAY_RATE)
        new_a = prev_a * EMOTION_DECAY_RATE + target_arousal * (1 - EMOTION_DECAY_RATE)
        new_d = prev_d * EMOTION_DECAY_RATE + target_dominance * (1 - EMOTION_DECAY_RATE)

        # V6: 写入 ActivationField
        if activation is not None:
            activation.valence = new_v
            activation.arousal = new_a
            activation.dominance = new_d

        # 向后兼容：更新实例变量
        self.valence = new_v
        self.arousal = new_a
        self.dominance = new_d
