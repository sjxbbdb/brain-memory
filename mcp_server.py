...
async def list_tools() -> list[types.Tool]:
    return [
        ...
        types.Tool(
            name="memory_search",
            description="五维认知检索——综合语义、目标、情绪、时间、因果五维度智能搜索已有记忆。",
            inputSchema={
                "type": "object",
                "properties": {
        ...
        types.Tool(
            ...
            description="执行睡眠巩固——刷新情绪权重、清理低权记忆、检测冲突、修正记忆。类比人脑睡眠时的记忆整理过程。",
            inputSchema={
                "type": "object",
                "properties": {
                    "phases": {"type": "array", "items": {"type": "integer"}, "description": "阶段: 0=权重自适应,1=评分刷新,2=分诊清理,25=冲突仲裁,3=记忆修正"},
                },
                "required": [],
            },
        ),
...
async def _search(a: dict) -> list[types.TextContent]:
    results = await retrieval.cognitive_search(RetrievalQuery(
