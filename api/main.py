"""Brain Memory v4.0 API — FastAPI + WebSocket.
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Brain v4.0 API started")

    yield

    await _brain.sleep()
    broadcaster.cancel()
    logger.info("Brain v4.0 API stopped")
...
app = FastAPI(title="Brain Memory v4.0", version="4.0.0", lifespan=lifespan)
...
async def post_input(req: InputRequest):
    ...
    brain = get_brain()
    return await brain.process_input(
        text=req.text,
        source=req.source,
        goal=req.goal,
    )
