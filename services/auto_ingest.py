...
async def check_duplicate(text: str, title: str, threshold: float = 0.55) -> dict:
    ...
    try:
        ...
        cursor = await db.execute(
            "SELECT id, title, content FROM memories WHERE archived = 0 ORDER BY created DESC LIMIT 50"
        )
        ...
        for row in rows:
            ...
            existing_fp = _text_fingerprint(existing_text)
            if existing_fp == fingerprint:
                return {"is_duplicate": True, "matched_id": row["id"], "overlap": 1.0}
        
        # 标题重叠检测
        overlap = _title_overlap(title, existing_titles)
        if overlap >= threshold:
            return {"is_duplicate": True, "matched_id": rows[0]["id"] if rows else None, "overlap": round(overlap, 2)}
        
        return {"is_duplicate": False, "matched_id": None, "overlap": round(overlap, 2)}
    finally:
        await db.close()
